...
block/.style={draw, rounded corners=2pt, minimum width=1.9cm, minimum height=0.66cm, align=center, fill=gray!8, font=\scriptsize},
loss/.style={draw, rounded corners=2pt, minimum width=1.85cm, minimum height=0.62cm, align=center, fill=orange!12, font=\scriptsize},
prior/.style={draw, rounded corners=2pt, minimum width=1.8cm, minimum height=0.62cm, align=center, fill=blue!8, font=\scriptsize},
data/.style={draw, rounded corners=2pt, minimum width=1.85cm, minimum height=0.62cm, align=center, fill=green!8, font=\scriptsize},
arrow/.style={-Latex, thick}
]
\node[data] (hq) {HQ image\\$I_{\mathrm{gt}}$};
\node[data, below=0.62cm of hq] (deg) {degradation\\blur/noise/JPEG};
\node[data, below=0.62cm of deg] (lq) {LQ input\\$I_{\mathrm{LQ}}$};
\node[block, right=0.75cm of lq] (enc) {LQ encoder\\multi-scale feats};
\node[block, right=of enc] (tr) {lookup transformer\\code logits};
\node[prior, above=0.72cm of tr] (idx) {latent target\\code indices};
\node[prior, right=of tr] (cb) {VQ codebook\\1024 atoms};
\node[block, right=of cb] (dec) {decoder\\HQ image};
\node[data, right=of dec] (out) {$I_{\mathrm{rec}}$};
\node[loss, below=0.72cm of cb] (code) {CE + feature\\code losses};
\node[loss, below=0.72cm of dec] (pix) {pixel + LPIPS\\GAN loss};
\node[loss, below=0.72cm of out] (freq) {log FFT magnitude\\$L_{\mathrm{freq}}$};
\node[loss, right=0.55cm of freq] (total) {total loss\\weighted sum};
\node[prior, above=0.72cm of cb] (vq) {fixed HQ VQGAN\\codebook prior};
\draw[arrow] (hq) -- (deg);
\draw[arrow] (deg) -- (lq);
\draw[arrow] (lq) -- (enc);
\draw[arrow] (enc) -- (tr);
...
\draw[arrow] (hq) -| (freq);
\draw[arrow] (out) -- (freq);
\draw[arrow] (code) -- (total);
\draw[arrow] (pix) -- (total);
\draw[arrow] (freq) -- (total);
\end{tikzpicture}
