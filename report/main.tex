...
\draw[arrow] (out) -- (freq);
\end{tikzpicture}
\caption{Architecture of the spectral-aware CodeFormer fine-tuning setup. The backbone remains a CodeFormer-style VQ codebook and code lookup transformer. The supervision branch compares log Fourier magnitudes of the reconstruction and ground truth.}
...
\midrule
Baseline CodeFormer fine tuning & $\lambda_{\mathrm{freq}}=0$ & Same architecture without spectral supervision & Anchor row for all gains.\\
Spectral fine tuning & $\lambda_{\mathrm{freq}}=0.1$ & Fourier magnitude supervision & Lower Freq-L1 with improved LPIPS and stable identity.\\
Low spectral weight & $\lambda_{\mathrm{freq}}=0.05$ & Mild frequency regularization & Small improvement with nearly no artifacts.\\
High spectral weight & $\lambda_{\mathrm{freq}}=0.2$ & Sensitivity to over-sharpening & Stronger edges but visible ringing and noise amplification.\\
Dataset transfer & DIV2K subset & Face-specific versus general behavior & Smaller gains than aligned-face restoration.\\
\bottomrule
\end{tabular}
\caption{Ablation matrix used to interpret the spectral loss contribution.}
\label{tab:ablations}
\end{table*}
...
The report frames the method as a controlled extension rather than a replacement for CodeFormer. In the best setting, the spectral term acts like a gentle regularizer for detail preservation. In the high-weight setting, it becomes an artifact amplifier. The ablation matrix in Table~\ref{tab:ablations} captures this behavior.
