...
@LOSS_REGISTRY.register()
class FrequencyL1Loss(nn.Module):
    """L1 loss on Fourier magnitudes for spectral-detail supervision.

    The image tensors are expected in the same normalized range as the pixel
    loss. FFT magnitudes are compared after log compression so the loss remains
    stable while still responding to high-frequency structure.
    """

    def __init__(self, loss_weight=1.0, reduction='mean', log_amplitude=True):
        super(FrequencyL1Loss, self).__init__()
